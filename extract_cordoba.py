...
def get_recent_cordoba_urls():
    ...
    for _ in range(20):
        if d.weekday() == 1:  # martes
            ...
            y = d.strftime("%Y")
            m = d.strftime("%m")
            # URL real: /wp-content/uploads/YYYY/MM/Acta-Mesa-de-Cereales-YYYYMMDD.pdf
            candidates[date_str] = f"https://camaracordoba.com/wp-content/uploads/{y}/{m}/Acta-Mesa-de-Cereales-{date_compact}.pdf"
        d -= timedelta(days=1)
    return candidates
